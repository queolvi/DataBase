INSERT INTO Patients(patient_id, patient_fio, patient_birthday, patient_gender, service_discount, patient_address)
VALUES 
(0, 'Горбин', 2000/01/01, 'M', 200.50 , 'Ул. Нестерова')
,(1, 'Стукачёв', 2000/02/02, 'M', 100.25 , 'Ул. Грачёва')
,(2, 'Сишарпов', 2000/03/03, 'M', 195.50 , 'Ул. Храпова')
,(3, 'Иванов', 2000/04/04, 'M', 225.35 , 'Ул. Стерлитамакская')
,(4, 'Носов', 2000/05/05, 'M', 115.75 , 'Ул. Гробова')
,(5, 'Ремонтов', 2000/06/06, 'M', 200.15 , 'Ул. Копьева')

INSERT INTO Specialties(specialty_id, specialty) 
VALUES
(0, 'Хирург')
,(1, 'Терапевт')
,(2, 'Анестезиолог')
,(3, 'Педиатр')
,(4, 'Ортопед')
,(5, 'Радиолог')
,(6, 'Невролог')
,(7, 'Кардиолог')

INSERT INTO Categories(category_number, category_name, price_per_visit) 
VALUES
(0, 'Первичный прием', 300.00),
(1, 'Повторный прием', 150.00),
(2, 'Консультация', 200.00),
(3, 'Экстренный прием', 500.00),
(4, 'Температурный контроль', 50.00),
(5, 'Обследование', 400.00);



INSERT INTO Diagnoses(diagnosis_code, diagnosis_name) 
VALUES
(0, 'Грипп'),
(1, 'ОРВИ'),
(2, 'Пневмония'),
(3, 'Диабет'),
(4, 'Гипертония'),
(5, 'Аллергия');

INSERT INTO Doctors(doctor_id, doctor_fio, specialty_id, category_number) 
VALUES
(0, 'Петров П.П.', 0, 0),
(1, 'Сидорова С.С.', 1, 1),
(2, 'Кузнецов К.К.', 2, 2),
(3, 'Иванова И.И.', 3, 3),
(4, 'Морозов М.М.', 4, 4),
(5, 'Тихонов Т.Т.', 5, 5);

INSERT INTO Appointments(appointment_id, doctor_id, patient_id, purpose, appointment_date, coupon_number, visit_cost, doctor_category) 
VALUES
(0, 0, 0, 'Консультация по гриппу', '2023-10-01', 12345, 300.00, 0),
(1, 1, 1, 'Диагностика ОРВИ', '2023-10-02', 12346, 150.00, 1),
(2, 2, 2, 'Лечение пневмонии', '2023-10-03', 12347, 400.00, 2),
(3, 3, 3, 'Обследование диабета', '2023-10-04', 12348, 500.00, 3),
(4, 4, 4, 'Консультация по гипертонии', '2023-10-05', 12349, 200.00, 4),
(5, 5, 5, 'Обследование аллергии', '2023-10-06', 12350, 50.00, 5);

INSERT INTO MedicalCards(med_card_id, patient_id, appointment_date, opening_date, diagnosis_code, appointment_id) 
VALUES
(0, 0, '2023-10-01', '2023-10-01', 0, 0),
(1, 1, '2023-10-02', '2023-10-02', 1, 1),
(2, 2, '2023-10-03', '2023-10-03', 2, 2),
(3, 3, '2023-10-04', '2023-10-04', 3, 3),
(4, 4, '2023-10-05', '2023-10-05', 4, 4),
(5, 5, '2023-10-06', '2023-10-06', 5, 5);

INSERT INTO MedicalRecordEntries(record_number, med_card_id, patient_id, diagnosis_code, doctor_fio) 
VALUES
(0, 0, 0, 0, 'Петров П.П.'),
(1, 1, 1, 1, 'Сидорова С.С.'),
(2, 2, 2, 2, 'Кузнецов К.К.'),
(3, 3, 3, 3, 'Иванова И.И.'),
(4, 4, 4, 4, 'Морозов М.М.'),
(5, 5, 5, 
